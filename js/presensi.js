// ============================================================
// PRESENSI.JS - v3.3.0 (LOW-END OPTIMIZED + ALL FIXES)
// ============================================================
// CHANGELOG v3.3.0:
// ✅ Fixed: checkTodayStatus() detects TERLAMBAT variants
// ✅ Fixed: updateButtonStates() uses cache + debounce (70% CPU ↓)
...
// ✅ Added: DeviceProfile detection (3-tier: low/mid/high)
// ✅ Added: Status cache with TTL (5s)
// ✅ Added: Debounced updateButtonStates (100ms)
// ✅ Added: Memory cleanup in compressImage
// ✅ Added: Offline queue support
// ✅ Added: Skip shadow blur & laser line on low-end
// ✅ Added: Lower canvas FPS on low-end (15 vs 60)
// ✅ Added: Lower image quality on low-end (0.35 vs 0.5)
// ✅ Added: Push notification scheduling (LAPKIN 16:30 & 18:00)
// ============================================================
...
async function fetchWithCors(url, options = {}) {
    ...
    try {
        ...
        if (!isPost && (error.message.includes('Failed to fetch') || 
...
// Time constants akan di-override oleh DeviceProfile di section 9
let TIME_CONSTANTS = {
    ONE_DAY_MS: 86400000,
...
try {
    ...
    if (el) {
        el.setAttribute('href', URL.createObjectURL(
...
// ============================================================
// 7. DEVICE PROFILE (3-TIER: LOW / MID / HIGH)
// ============================================================
const DeviceProfile = (() => {
    ...
    // Apply class untuk CSS targeting
    if (tier === 'low') {
        document.documentElement.classList.add('low-end-device');
    }
    
    console.info(`📱 Device Profile: ${tier.toUpperCase()} (RAM: ${ram}GB, Cores: ${cores}, Network: ${isSlowNetwork ? 'SLOW' : 'OK'})`);
    
    return { tier, config: configs[tier], cores, ram, isSlowNetwork };
})();

// Override TIME_CONSTANTS based on DeviceProfile
TIME_CONSTANTS.FETCH_TIMEOUT_MS = DeviceProfile.config.fetchTimeout;
TIME_CONSTANTS.SUBMIT_TIMEOUT_MS = DeviceProfile.config.submitTimeout;
TIME_CONSTANTS.GPS_TIMEOUT_MS = DeviceProfile.config.gpsTimeout;
TIME_CONSTANTS.BUTTON_UPDATE_INTERVAL_MS = DeviceProfile.config.buttonUpdateInterval;
TIME_CONSTANTS.AUTO_REFRESH_INTERVAL_MS = DeviceProfile.config.refreshInterval;
...
function _showToastInternal({ title, message, type }) {
    ...
    // Skip audio di low-end
    if (DeviceProfile.config.enableAudio) {
...
function checkTodayStatus(pid) {
    ...
    todayRecords.forEach(r => {
        ...
        // ✅ Better detection for TERLAMBAT variants
...
function updateButtonStates() {
    ...
    // Debounce: 200ms di low-end, 100ms di high-end
    ...
    navigator.geolocation.getCurrentPosition(
        (p) => {
            if (p.coords.accuracy > 250) {
                if (DeviceProfile.config.enableAudio) sndError.play();
...
function initMap() {
    ...
    // Skip label tiles di low-end untuk hemat network
    if (DeviceProfile.tier !== 'low') {
...
function setS(el, st) {
    ...
    if (st === 'IZIN') {
        if (status.hasIzin) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Ditolak", "❌ Anda sudah IZIN hari ini. Hanya boleh 1x sehari.", "error"); return; }
        if (status.hasSpecial) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Ditolak", `❌ Anda sudah punya status khusus hari ini (${status.specialType.toUpperCase()}).`, "error"); return; }
        if (status.hasAnyHadir) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Ditolak", "❌ Anda sudah HADIR hari ini. Tidak bisa IZIN.", "error"); return; }
    }
    
    if (st === 'SAKIT') {
        if (status.hasSakit) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Ditolak", "❌ Anda sudah SAKIT hari ini. Hanya boleh 1x sehari.", "error"); return; }
        if (status.hasSpecial) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Ditolak", `❌ Anda sudah punya status khusus hari ini (${status.specialType.toUpperCase()}).`, "error"); return; }
        if (status.hasAnyHadir) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Ditolak", "❌ Anda sudah HADIR hari ini. Tidak bisa SAKIT.", "error"); return; }
    }
    
    if (st === 'DINAS') {
        if (status.hasDinas) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Ditolak", "❌ Anda sudah DINAS hari ini. Hanya boleh 1x sehari.", "error"); return; }
        if (status.hasSpecial) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Ditolak", `❌ Anda sudah punya status khusus hari ini (${status.specialType.toUpperCase()}).`, "error"); return; }
        if (status.hasAnyHadir) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Ditolak", "❌ Anda sudah HADIR hari ini. Tidak bisa DINAS.", "error"); return; }
    }
    
    if (st === 'HADIR') {
        if (status.hasAnyHadir) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Sudah Absen", "❌ Anda sudah HADIR/QR HADIR hari ini. Hanya boleh 1x.", "error"); return; }
        if (status.hasSpecial) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Ditolak", `❌ Anda sudah ${status.specialType.toUpperCase()} hari ini. Tidak bisa HADIR.`, "error"); return; }
        if (status.hasAnyPulang) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Ditolak", "❌ Anda sudah PULANG hari ini. Tidak bisa HADIR.", "error"); return; }
    }
    
    if (st === 'PULANG') {
        if (status.hasAnyPulang) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Sudah Absen", "❌ Anda sudah PULANG/QR PULANG hari ini.", "error"); return; }
        if (!status.hasAnyHadir) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Urutan Salah", "❌ Harap HADIR terlebih dahulu sebelum PULANG.", "error"); return; }
        if (status.hasQRHadir) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Pairing Salah", "❌ Anda QR HADIR pagi ini. Gunakan QUICK RESPONSE untuk QR PULANG.", "warning"); return; }
    }
    ...
    if (st === 'QUICK RESPONSE') {
        ...
        if (isMorning) {
            if (status.hasAnyHadir) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Sudah Absen", "❌ Anda sudah HADIR/QR HADIR hari ini. Hanya boleh 1x.", "error"); return; }
            if (status.hasSpecial) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Ditolak", `❌ Anda sudah ${status.specialType.toUpperCase()} hari ini.`, "error"); return; }
            if (status.hasAnyPulang) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Ditolak", "❌ Anda sudah PULANG hari ini. Tidak bisa QR HADIR.", "error"); return; }
        } else {
            if (status.hasAnyPulang) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Sudah Absen", "❌ Anda sudah PULANG/QR PULANG hari ini.", "error"); return; }
            if (!status.hasAnyHadir) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Urutan Salah", "❌ Harap HADIR/QR HADIR terlebih dahulu.", "error"); return; }
            if (status.hasHadirBiasa && !status.hasQRHadir) { if (DeviceProfile.config.enableAudio) sndError.play(); showToast("Pairing Salah", "❌ Anda HADIR biasa pagi ini. Gunakan tombol PULANG biasa.", "warning"); return; }
        }
    }
...
async function submitWithRetry(attempt = 1, trxId = null) {
    ...
    try {
        ...
        if (j.status === 'success') {
            ...
            if (DeviceProfile.config.enableAudio) sndSuccess.play().catch(() => {});
            ...
            if (j.message && (j.message.includes('duplikat') || j.message.includes('sudah'))) {
                ...
                if (DeviceProfile.config.enableAudio) sndError.play().catch(() => {});
    ...
    img.onload = async () => {
        ...
        try {
            ...
            tempImg.onload = () => {
                ...
                kB64 = d;
                setLoading(false);
                if (DeviceProfile.config.enableAudio) sndShutter.play();
